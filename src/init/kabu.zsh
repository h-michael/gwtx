# kabu init (zsh)
if command -v ::KABU:: >/dev/null 2>&1; then
  source <(::KABU:: completions zsh)
fi
...
kabu() {
  if [[ "${1:-}" == "cd" ]] && (( $# == 1 )); then
    ...
    local dest
    dest=$(__kabu_cmd path) || return $?
    if [[ -n "$dest" ]]; then
      builtin cd "$dest"
    fi
  elif [[ "${1:-}" == "add" ]]; then
    local cd_to
    cd_to=$(__kabu_cmd config get auto_cd.after_add 2>/dev/null) || cd_to=""

    # Capture output while displaying it
    local tmpfile
    tmpfile=$(mktemp)
    __kabu_cmd "$@" 2>&1 | tee "$tmpfile"
    ...
    local cd_to
    cd_to=$(__kabu_cmd config get auto_cd.after_remove 2>/dev/null) || cd_to=""
    local main_path
    main_path=$(__kabu_cmd path --main 2>/dev/null) || main_path=""
...
function __kabu_trust_check() {
  ...
  # Get .kabu/config.yaml mtime (Linux: stat -c '%Y', macOS: stat -f '%m')
  ...
  if [[ "$root" != "$__kabu_trust_root" ]] || \
     [[ "$current_mtime" != "$__kabu_trust_config_mtime" ]]; then
    __kabu_trust_root="$root"
    __kabu_trust_config_mtime="$current_mtime"
    __kabu_trust_state=""
  fi

  if ::KABU:: trust --check "$root"; then
    __kabu_trust_state="trusted"
    return
  fi
...
function __kabu_precmd() {
  __kabu_trust_check
}

if typeset -a precmd_functions >/dev/null 2>&1; then
  if (( ! ${precmd_functions[(I)__kabu_precmd]} )); then
    precmd_functions+=(__kabu_precmd)
  fi
else
  precmd() { __kabu_trust_check }
fi

function __kabu_chpwd() {
  __kabu_trust_check
}

if typeset -a chpwd_functions >/dev/null 2>&1; then
  if (( ! ${chpwd_functions[(I)__kabu_chpwd]} )); then
    chpwd_functions+=(__kabu_chpwd)
  fi
else
  chpwd() { __kabu_trust_check }
fi
