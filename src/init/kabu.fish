...
function kabu
  if test (count $argv) -eq 1; and test "$argv[1]" = "cd"
    ...
    set -l dest (__kabu_cmd path)
    if test -n "$dest"
      builtin cd "$dest"
    end
  else if test (count $argv) -ge 1; and test "$argv[1]" = "add"
    set -l cd_to (__kabu_cmd config get auto_cd.after_add 2>/dev/null)

    # Capture output while displaying it
    set -l tmpfile (mktemp)
    __kabu_cmd $argv 2>&1 | tee $tmpfile
    ...
    set -l cd_to (__kabu_cmd config get auto_cd.after_remove 2>/dev/null)
    set -l main_path (__kabu_cmd path --main 2>/dev/null)
...
function __kabu_trust_check --on-event fish_prompt --on-variable PWD
  set -l root (git rev-parse --show-toplevel 2>/dev/null)
  if test -z "$root"
    set -g __kabu_trust_root ""
    set -g __kabu_trust_config_mtime ""
    set -g __kabu_trust_state ""
    return
  end

  # Get .kabu/config.yaml mtime (Linux: stat -c '%Y', macOS: stat -f '%m')
  set -l config_path "$root/.kabu/config.yaml"

  # Skip if .kabu/config.yaml doesn't exist
  ...
  if test "$root" != "$__kabu_trust_root"; or test "$current_mtime" != "$__kabu_trust_config_mtime"
    set -g __kabu_trust_root "$root"
    set -g __kabu_trust_config_mtime "$current_mtime"
    set -g __kabu_trust_state ""
  end

  # Run trust check and capture exit code
  if ::KABU:: trust --check "$root" 2>/dev/null
    set -g __kabu_trust_state trusted
    return
  end
